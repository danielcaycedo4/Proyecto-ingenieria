...
void main(void) {
    settings();
    writeI2C(SlaveAddress1,0xB0,name,16);//0xB5=ubicacion del registro
    writeI2C(SlaveAddress1,0xFF,&name[5],1);//envia solo un dato del puntero o vector(&)
    readI2C(SlaveAddress1,0xB0,dataSaved,16);
    while(1){
        Servos();
...
void Servos(void){
    ...
    /*for(int j=0; j<4; j++){ //Pines o canales PWM de la placa
        ...
        __delay_ms(1000);
        pos = map (90, 0 , 180 , SERVOMIN, SERVOMAX);
        data2[2] = pos;
        data2[3] = pos >> 8;
        writeI2C(I2C_ADDRESS_PCA9685, PCA9685_LED0_ON_L + (4 * DriverPin), data2, 4);
        __delay_ms(1000);
...
void abre_griper(void){
    ...
    data2[2] = pos;
    data2[3] = pos >> 8;
    writeI2C(I2C_ADDRESS_PCA9685, PCA9685_LED0_ON_L + (4 * 0), data2, 4);
    __delay_ms(1000);
}
...
void sube_junta(void){
    ...
    data2[2] = pos;
    data2[3] = pos >> 8;
    writeI2C(I2C_ADDRESS_PCA9685, PCA9685_LED0_ON_L + (4 * 1), data2, 4);
    __delay_ms(1000);
}
...
void gira_izquierda(void){
    ...
    data2[2] = pos;
    data2[3] = pos >> 8;
    writeI2C(I2C_ADDRESS_PCA9685, PCA9685_LED0_ON_L + (4 * 2), data2, 4);
    __delay_ms(1000);
}
