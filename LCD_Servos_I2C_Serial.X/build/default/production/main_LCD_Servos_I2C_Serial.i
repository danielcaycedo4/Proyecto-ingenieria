# 1 "main_LCD_Servos_I2C_Serial.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 288 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "main_LCD_Servos_I2C_Serial.c" 2
...
# 2 "main_LCD_Servos_I2C_Serial.c" 2
...
# 3 "main_LCD_Servos_I2C_Serial.c" 2
...
# 49 "main_LCD_Servos_I2C_Serial.c"
void settings(void);
void algoritm(void);

void __attribute__((picinterrupt(("")))) receive(void);
void start(void);
...
int i, pos, dato, DriverPin = 0, servos = 5,z = 0, Angle, coor = 0, Coordinate_y, Coordinate_x, Coordinate_z;
int angle_servos[6] = {90,90,90,90,90,90};
unsigned char data1, data2[4], d1, flag=0, d;
char text[40], angle[3], texto_LCD[16], coordinate_x[3], coordinate_y[3], coordinate_z[3];
...
void setPWMFreq(float freq){
    ...
    readI2C(0x80, 0x00, &data1, 1);

    unsigned char newmode = (data1 & ~0x80) | 0x10;
    writeI2C(0x80, 0x00, &newmode,1);
    writeI2C(0x80, 0xFE, &prescale,1);
    writeI2C(0x80, 0x00, &data1,1);
    _delay((unsigned long)((5)*(8000000/4000.0)));

    d1=(data1 | 0x80 | 0x20);
    writeI2C(0x80, 0x00, &d1, 1);
}
...
void mover_servo(int servo,int angulo){
    pos = map (angulo, 0 , 180 , 110, 510);
    data2[2] = pos;
    data2[3] = pos >> 8;
    writeI2C(0x80, 0x06 + (4 * servo), data2, 4);
